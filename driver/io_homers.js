...
var io = {
	...
	read: function (item) {
		...
		$.ajax({
            url : io.url + "getfitem?id=" + item +"&limit=1",
            type : "GET",
            async : true,
            cache : true
		}).done(function(state) {
...
/*
	...
	init: function (address, port) {
		...
		io.url = "http://" + address + (port ? ":" + port : '') + "/rpc/";
	},
	...
	run: function (realtime) {
        ...
        if (widget.listeners().length) {
            ...
            for (var i = 0; i < widget.listeners().length; i++) {
				$.ajax({
					url		: io.url + "getfitem?id=" + items[i] +"&limit=1",
					type	: "GET",
					async	: true,
					cache	: true
				}).done(function(ohItem) {
					io.debug && console.debug(ohItem);
					var item = ohItem[0].item;
					//io.itemType[item] = ohItem.type;
					var val = ohItem[0].val;//io.convertState(item, ohItem.state);
					io.debug && console.debug("io.run: widget.update(item = " + item + ", value = " + val + ")");
					widget.update(item.toString(), val);
				}).error(notify.json);
            }
        }
	...
	plot: {
		...
		get: function(plotItem) {
			...
			var url = io.url + "getfitem?id=" + item + "&limit="+limit; // + "?starttime=" + starttime.toISOString();
			...
			$.ajax({
				...
				io.debug && console.debug(plotData);
